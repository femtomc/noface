...
const ComplianceResult = compliance_mod.ComplianceResult;
...
pub const AgentLoop = struct {
    ...
    fn runIteration(self: *AgentLoop) !bool {
        ...
        var compliance_checker = if (self.state) |*s|
            ComplianceChecker.initWithState(self.allocator, s)
        else
            ComplianceChecker.init(self.allocator);
        var baseline = try compliance_checker.captureBaseline();
        ...
        var last_violation: ?ComplianceResult = null;
        ...
        while (attempt < retry_config.max_attempts) : (attempt += 1) {
            ...
            var compliance = try compliance_checker.verify(issue_id, &baseline);

            // Log manifest instrumentation metrics (predicted vs actual files)
            compliance_mod.logInstrumentation(&compliance, self.allocator, self.config.verbose);
            ...
            if (!compliance.compliant) {
                ...
                try compliance_checker.rollback(&compliance);
    ...
    fn buildStricterPrompt(self: *AgentLoop, issue_id: []const u8, violation_result: ComplianceResult) ![]const u8 {
