...
pub const AgentLoop = struct {
    ...
    pub fn run(self: *AgentLoop) !void {
        ...
        while (!self.isInterrupted()) {
            ...
            // Run the greedy scheduler - it handles all parallel execution
            const did_work = try self.runScheduler();

            if (!did_work) {
                // No work done - in event-driven mode, trigger planner
                ...
                if (self.config.enable_planner and self.config.planner_mode == .event_driven and !planner_ran_this_iter) {
                    self.logInfo("No ready issues, triggering planner (event-driven)", .{});
                    if (!try self.runPlannerPass()) {
                        self.logInfo("Agent loop stopping after failed planner pass", .{});
                        break;
                    }
                    ...
                    // Retry scheduler after planner creates new work
                    const retry_result = try self.runScheduler();
                    if (retry_result) {
                        continue;
                    }
                }
            ...
            // Sync to GitHub after scheduler completes
            try self.syncGitHub();
    ...
    /// Run the greedy scheduler to execute ready issues in parallel.
    /// Returns true if any work was done, false if no ready issues were found.
    fn runScheduler(self: *AgentLoop) !bool {
        var pool = &(self.worker_pool orelse return false);

        self.logInfo("=== Scheduler Iteration {d} ===", .{self.iteration});

        // Run the greedy scheduler
        const did_work = try pool.runSchedulerLoop(self.config.dry_run);

        // Save state after scheduler completes
        self.saveState();
    ...
    fn runPlannerPass(self: *AgentLoop) !bool {
        ...
        while (attempt < retry_config.max_attempts) : (attempt += 1) {
            ...
            if (last_exit_code == 0) {
                ...
                // The greedy scheduler will use these for conflict detection
