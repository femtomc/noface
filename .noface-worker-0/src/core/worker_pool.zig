...
pub const WorkerPool = struct {
    ...
    /// Run the greedy scheduler loop.
    /// Continuously assigns ready issues to idle workers until no work remains.
    /// Returns true if any work was done, false if no ready issues were found.
    pub fn runSchedulerLoop(self: *WorkerPool, dry_run: bool) !bool {
        logInfo("Starting greedy scheduler with {d} worker(s)", .{self.state.num_workers});
        ...
        while (true) {
            ...
            try self.processPendingResults();

            // 5. Get list of currently running issues (for conflict checking)
            var running_issues = std.ArrayListUnmanaged([]const u8){};
            defer running_issues.deinit(self.allocator);
            for (0..self.state.num_workers) |i| {
                if (self.worker_processes[i] != null) {
                    if (self.state.workers[i].current_issue) |issue| {
                        try running_issues.append(self.allocator, issue);
                    }
                }
            }

            // 6. Try to assign ready issues to idle workers
            const assigned = try self.assignReadyIssues(&running_issues, dry_run);
            if (assigned > 0) {
                did_work = true;
            }

            // 7. Check if we should continue
            const running_count = self.runningWorkerCount();
            if (running_count == 0 and assigned == 0) {
                // No workers running and no new assignments - check if more work exists
                const has_ready = self.hasReadyIssues() catch false;
                if (!has_ready) {
                    logInfo("No more ready issues, scheduler stopping", .{});
                    break;
                }
                // There are ready issues but they might all conflict with each other
        ...
        if (did_work) {
            logSuccess("Scheduler completed", .{});
        }
        return did_work;
    }

    /// Process pending results from completed workers
    fn processPendingResults(self: *WorkerPool) !void {
    ...
    /// Assign ready issues to idle workers, avoiding conflicts with running issues.
    /// Returns the number of issues assigned.
    fn assignReadyIssues(self: *WorkerPool, running_issues: *std.ArrayListUnmanaged([]const u8), dry_run: bool) !u32 {
        // Get list of ready issues from bd
        var ready_result = process.shell(self.allocator, "bd ready --json 2>/dev/null | jq -r '.[].id'") catch return 0;
        ...
        while (lines.next()) |line| {
            const issue_id = std.mem.trim(u8, line, " \t\r");
            if (issue_id.len == 0) continue;

            // Skip if issue is already running
            var already_running = false;
            for (running_issues.items) |running| {
                if (std.mem.eql(u8, running, issue_id)) {
            ...
            // Check for manifest conflicts with running issues
            var conflicts = false;
            for (running_issues.items) |running| {
                if (self.state.issuesConflict(issue_id, running)) {
                    conflicts = true;
                    break;
                }
            }
            ...
            if (self.findIdleWorkerSlot()) |worker_id| {
                if (dry_run) {
                    logInfo("[DRY RUN] Would assign issue {s} to worker {d}", .{ issue_id, worker_id });
                    assigned += 1;
                    continue;
                }
                ...
                // Add to running list for conflict checking of subsequent issues
                try running_issues.append(self.allocator, self.state.workers[worker_id].current_issue.?);
            } else {
    ...
    /// Check if there are any ready issues available
    fn hasReadyIssues(self: *WorkerPool) !bool {
        var ready_result = try process.shell(self.allocator, "bd ready --json 2>/dev/null | jq 'length'");
        defer ready_result.deinit();

        if (!ready_result.success()) return false;

        const count_str = std.mem.trim(u8, ready_result.stdout, " \t\n\r");
